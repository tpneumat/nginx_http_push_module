...
static void ngx_http_push_publisher_body_handler(ngx_http_request_t * r) { 
	...
	switch(method) {
		...
		case NGX_HTTP_POST:
			...
			msg->message_time=(r->headers_in.if_modified_since == NULL) ? ngx_time() : ngx_http_parse_time(r->headers_in.if_modified_since->value.data, r->headers_in.if_modified_since->value.len);
